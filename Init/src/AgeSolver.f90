SUBROUTINE AgeSolver( Model,Solver,dt,TransientSimulation )
  ...
  DO k=1,NM
     ...
     IF(.NOT.IsThere) THEN
        IsThere=ClosestPointInBoundary(Element, ElementNodes, NodeIndexes, n, xm, LocalCoordinates,eps, &
             k,Solver,NoNeigh,NeighList)
     END IF
     ...
     IsThere=PointInDomain(Element, ElementNodes, NodeIndexes, n, xd, LocalCoordinates,eps, &
          k,Solver,NoNeigh,NeighList)
     IF(.NOT.IsThere) THEN
        IsThere=ClosestPointInBoundary(Element, ElementNodes, NodeIndexes, n, xd, LocalCoordinates,eps, &
             k,Solver,NoNeigh,NeighList)
     END IF
